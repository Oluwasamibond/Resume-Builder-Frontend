import TEMPLATE_ONE_IMG from "../assets/template-1.png";
import TEMPLATE_TWO_IMG from "../assets/template-2.png";
import TEMPLATE_THREE_IMG from "../assets/template-3.png";

export const resumeTemplates = [
  {
    id: "01",
    thumbnailImg: TEMPLATE_ONE_IMG,
    colorPaletteCode: "themeOne",
  },
  {
    id: "02",
    thumbnailImg: TEMPLATE_TWO_IMG,
    colorPaletteCode: "themeT",
  },
  {
    id: "03",
    thumbnailImg: TEMPLATE_THREE_IMG,
    colorPaletteCode: "themeThree",
  },
];

export const themeColorPalette = {
  themeOne: [
    ["#EBFDFF", "#A1F4FD", "#CEFAFE", "#0088DB", "#4A5565"],

    ["#E9F8F8", "#B4EFE7", "#93E2DA", "#2AC9A0", "#3D4C5A"],
    ["#F5F4FF", "#E0DBFF", "#C9C2F8", "#8579D1", "#4B4B5C"],
    ["#F0FAFF", "#D6F0FF", "#AFDEFF", "#3399FF", "#445361"],
    ["#FFF5F7", "#FFE0EC", "#FAC6D4", "#F6729C", "#5A5A5A"],
    ["#F9FAFB", "#E4E7EB", "#CBD5E0", "#7F9CF5", "#2D3748"],

    ["#F4FFFD", "#D3FDF2", "#B0E904", "#34C790", "#384C48"],
    ["#FFF7F0", "#FFE6D9", "#FFD2BA", "#FF9561", "#4C4743"],
    ["#F9FCFF", "#E3F0F9", "#C0DDEE", "#6CA6CF", "#46545E"],
    ["#FFFDF6", "#FFF4D7", "#FFE7A0", "#FFD000", "#57534E"],
    ["#EFFCFF", "#C8F0FF", "#99E0FF", "#007BA7", "#2B3A42"],

    ["#F7F7F7", "#E4E4E4", "#CFCFCF", "#A4A4A4", "#222222"],
    ["#E3F2FD", "#90CAF9", "#A8D2F4", "#1E88E5", "#0D47A1"],
  ],
};

export const DUMMY_RESUME_DATA = {
  profileInfo: {
    profileImg: null,
    previewUrl: "",
    fullName: "John Doe",
    designation: "Senior Software Engineer",
    summary:
      "Passionate and result-driven developer with 6+ years of experience",
  },
  contactInfo: {
    email: "johndoe@example.com",
    phone: "+1234567890",
    location: "#12 Anywhere, Any City, Any Country",
    linkedin: "https://linkedin.com/johndoe",
    github: "https://github.com/johndoe",
    website: "https://johndoe.com",
  },
  workExperience: [
    {
      company: "Tech Solutions",
      role: "Senior Frontend Developer",
      startDate: "2022-03",
      endDate: "2025-04",
      description:
        "Leading the frontend team to build scaleable enterprise applications using React",
    },
    {
      company: "Coding Dev",
      role: "Full-Stack Developer",
      startDate: "2020-03",
      endDate: "2024-04",
      description:
        "Worked on cross-functional teams developing full-stack solutions with React",
    },
    {
      company: "Startup Company",
      role: "Junior Web Developer",
      startDate: "2018-06",
      endDate: "2019-02",
      description:
        "Built responsive websites for startups and small businesses",
    },
  ],
  education: [
    {
      degree: "M.Sc. Software Engineering",
      institution: "Tech University",
      startDate: "2021-08",
      endDate: "2023-06",
    },
    {
      degree: "B.Sc. Computer Science",
      institution: "State University",
      startDate: "2017-10",
      endDate: "2021-11",
    },
  ],
  skills: [
    { name: "Javascript", progress: 95 },
    { name: "React", progress: 90 },
    { name: "Node.js", progress: 85 },
    { name: "TypeScript", progress: 90 },
    { name: "MongoDB", progress: 90 },
  ],
  projects: [
    {
      title: "Project Manager App",
      description: "A task and team management app built with MERN stack",
      github: "https://github.com/johndoe/project-manager-app",
    },
    {
      title: "E-commerce Platform",
      description:
        "An e-commerce site built with Next.js and Paystack integration including user roles, authentication, and admin-dashboard",
      liveDemo: "https://ecommerce-demo.com",
    },
    {
      title: "Blog CMS",
      description: "A custom CMS for blogging using Express and React",
      github: "https://github.com/johndoe/blog-cms",
      liveDemo: "https://blogcms.com",
    },
  ],
  certifications: [
    {
      title: "Full Stack Web Developer",
      issuer: "Udemy",
      year: "2023",
    },
    {
      title: "React Advanced Certification",
      issuer: "Free Code Camp",
      year: "2023",
    },
  ],
  languages: [
    { name: "English", progress: 100 },
    { name: "Spanish", progress: 70 },
    { name: "French", progress: 50 },
  ],
  interests: ["Reading", "Open Source Contribution", "Hiking"],
};
